...
- [调整] 模型脚本变量说明及复制指南补充参考音频公网地址、长期地址优先与临时授权回退示例。

- [新增] 带音频主体在视频脚本执行前自动上传并去重，提供长期参考音频地址与主体关联，上传失败时阻止模型提交。

- [新增] 服务器同步改为同账号多设备按记录增量合并，纳入主体、密钥与渠道脚本，支持冲突选择并保留独立历史恢复入口。

- [新增] AI 工作台前端交互，支持生成偏好、参考图片与主体引用，并在我的资产中新增共享主体分类；暂未接入生成服务。

- [新增] AI 工作台接入视频任务创建、轮询和本地素材保存，主体图片、音频及归属上下文传入渠道脚本。

- [调整] 服务器备份登录改为用户账号模型，支持超管和普通用户角色，初始超管账号为 `dongxin`。
+ [修复] 服务器登录页不再预填默认账号，并将默认请求地址指向云端后端，避免登录请求发到前端站点返回 405。
+ [新增] 配置页在 WebDAV 旁新增「服务器同步」页签，支持登录服务器并将本地画布、素材与生成记录上传或从服务器备份下载。
+ [修复] 进入服务器同步前先固化当前标签页仍可读取的画布媒体，避免旧 Blob URL 在新同步页中失效导致上传失败。
