export default {
    ...
    modelPlugin: {
        referenceAudioGuide: "Reference audio and public URLs: audios is File[], not URLs; use form.append only when the provider accepts multipart uploads. When public audio URLs are required, prefer audioUrls[index]. The AI workbench uploads or reuses audio before running the script and supplies persistent URLs in the same order as audios. Canvas and other calls may leave audioUrls empty; use (await cloud.mediaUrl(audios[index], { kind: 'audio' })).url to obtain a temporary public URL. This requires signing in to the media server. Stop submission on failure; never substitute blob URLs, local paths, or File objects. Do not persist temporary URLs in subjects. Map each URL to the provider's documented reference-audio field; audioUrls is not a universal request body, and reference audio is distinct from the generateAudio output-audio toggle.",
